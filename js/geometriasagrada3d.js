...
const GRID_FLOOR_Y = -GRID_WALL_DISTANCE;
const GRID_CEILING_Y = GRID_WALL_DISTANCE;
...
function createRoomEdges() {
  ...
  const corners = [
    new THREE.Vector3(-half, GRID_FLOOR_Y, -half),
    new THREE.Vector3(half, GRID_FLOOR_Y, -half),
    new THREE.Vector3(half, GRID_FLOOR_Y, half),
    new THREE.Vector3(-half, GRID_FLOOR_Y, half),
    new THREE.Vector3(-half, GRID_CEILING_Y, -half),
    new THREE.Vector3(half, GRID_CEILING_Y, -half),
    new THREE.Vector3(half, GRID_CEILING_Y, half),
    new THREE.Vector3(-half, GRID_CEILING_Y, half)
  ];
...
function createBackgroundGrid() {
  ...
  floor.position.y = GRID_FLOOR_Y;
  ceiling.position.y = GRID_CEILING_Y;
...
if (typeof glow !== 'undefined' && glow) {
  ...
  sacredGroup.position.y = audioAverage * 20;
...
audioInput.addEventListener("change", async (e)=>{
  ...
  try {
    ...
    if (audioLabel) audioLabel.childNodes[0].textContent = file.name;
  } catch (err) {
    console.error("No se pudo reproducir el audio seleccionado", err);
  }
